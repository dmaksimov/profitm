...
class Campaign extends \ProfitMiner\Base\Models\Campaign
{
    ...
    public function scopeFilterByIndustry($query, IndustryType $industry)
    {
        return $query->where(function ($query) use ($industry) {
            $query->orWhere('industry_type_id', $industry->id);
        });
    }
    ...
    public static function searchByRequest(Request $request)
    {
        ...
        if ($request->has('industry')) {
            $query->filterByIndustry(IndustryType::findOrFail($request->input('industry')));
        }
        return $query;
    }
